...
func ParseYAMLData(fileData []byte, regex string, filter string, envsubst bool) (*Services, error) {
	...
	if len(services.Provider.Version) > 0 && !IsValidSchemaVersion(services.Provider.Version) {
		return nil, fmt.Errorf("%s are the only valid versions for the stack file - found: %s", ValidSchemaVersions, services.Provider.Version)
	}
